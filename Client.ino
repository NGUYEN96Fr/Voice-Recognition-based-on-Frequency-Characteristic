 ...
 * load           load (r0) (r1) ...            load 0 51 2 3          c      Load records
...
LiquidCrystal lcd(5, 7, 8, 9, 11, 13);
...
{ 
  ...
  if(ret >= 0){
    ...
    lcd.clear();
    lcd.setCursor(0,1);
    lcd.print("Train failed.");
    delay(3000);
  }
  else if(ret == -2){
    lcd.clear();
    lcd.setCursor(0,1);
    lcd.print("Train Timeout.");
    delay(3000);
  }
  printSeperator();
  return 0;
...
int cmdClear(int len, int paraNum)
{
  ...
  if(myVR.clear() == 0){
    ...
    lcd.clear();
    lcd.setCursor(0,0);
    lcd.print(F("Clear recognizer"));
    lcd.setCursor(0,1);
    lcd.print(F("failed or time out. "));
    printSeperator();
    delay(3000);
  }
  lcd.begin(16,2);
...
void printLoad(uint8_t *buf, uint8_t len)
{
  if(len == 0){
    lcd.clear();
    lcd.setCursor(0,0);
    lcd.print(F("Load Successfully."));
    return;
  }
  else{
    lcd.clear();
    lcd.setCursor(0,0);
    lcd.print(F("Load success: "));
    lcd.print(buf[0], DEC);
    delay(3000);
  }
  for(int i=0; i<len-1; i += 2){
    ...
    case 0xFC:
      lcd.setCursor(0,1);
      lcd.print(F("Record already in recognizer"));
      break;
    case 0xFD:
      lcd.print(F("Recognizer full"));
      break;
    case 0xFE:
      lcd.setCursor(0,1);
      lcd.print(F("Record untrained"));
      break;
    case 0xFF:
      lcd.setCursor(0,1);
      lcd.print(F("Value out of range"));
      break;
    default:
      lcd.setCursor(0,1);
      lcd.print(F("Unknown status"));
      break;
    }
    delay(4000);
  }
...
{   
  if(len == 0){
    lcd.clear();
    lcd.setCursor(0,0);
    lcd.print(F("Train Finish."));
    delay(3000);
    return;
  }
  else{
    lcd.clear();
    lcd.print(F("Train success: "));
    lcd.print(buf[0], DEC);
    delay(3000);
  }
